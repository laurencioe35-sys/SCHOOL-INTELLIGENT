...
from database.models import Base, Organization, User, Role
...
def ensure_demo_teacher_seed() -> None:
    """Crea la cuenta demo del docente para que el flujo real del
    frontend quede operativo inmediatamente al arrancar la app."""
    with SessionLocal() as db:
        org = db.query(Organization).filter(Organization.name == "Colegio Demo").first()
        if org is None:
            org = Organization(name="Colegio Demo")
            db.add(org)
            db.flush()

        user = db.query(User).filter(User.email == "demo.teacher@example.com").first()
        if user is None:
            user = User(
            ...
            user.full_name = "Docente Demo"
            user.hashed_password = hash_password("Clave123!")
