...
def require_real_provider() -> None:
    if not has_real_provider() and os.getenv("LLM_ALLOW_MOCK", "false").lower() != "true":
        raise RuntimeError(
            "No real LLM provider configured. Set GEMINI_API_KEY, ANTHROPIC_API_KEY, "
            "OPENAI_API_KEY, or ANTHROPIC_FALLBACK_API_KEY; mocks are test-only."
        )
...
def _mock_response(system_prompt: str, user_text: str) -> str:
    ...
    if '"message"' in system_prompt:
        return json.dumps({
            "message": f"He recibido la instrucción: {user_text.strip()[:300]}. Prepararé el apoyo pedagógico correspondiente.",
            "destination": "chat",
            "component": None,
        })
...
def _call_gemini(
    ...
    """Call Google's Gemini generateContent API using the REST contract."""
    import requests

    generation_config: dict = {
        # Gemini puede consumir parte del presupuesto en razonamiento antes
        # de emitir el JSON final. 300 tokens truncaba los agentes complejos
        # antes de la respuesta estructurada.
    ...
    request = {
        ...
        # Los agentes consumen la respuesta mediante json.loads(). Sin pedir
        # JSON explícitamente Gemini puede envolverlo en Markdown o responder
        # texto libre, dejando el pipeline sin una acción utilizable.
...
def _build_providers() -> list[dict]:
    ...
    if os.getenv("GEMINI_API_KEY"):
        ...
        providers.append({
            "name": "gemini_primary",
            "call": lambda sp, ut, schema=None, _k=api_key, _m=model: (
                _call_gemini(sp, ut, _k, _m)
                if schema is None
                else _call_gemini(sp, ut, _k, _m, schema)
            ),
        })
    ...
    if os.getenv("ANTHROPIC_API_KEY"):
        ...
        providers.append({
            ...
            "call": lambda sp, ut, schema=None, _k=api_key, _m=model: _call_anthropic(sp, ut, _k, _m),
        })
    ...
    if os.getenv("OPENAI_API_KEY"):
        ...
        providers.append({
            ...
            "call": lambda sp, ut, schema=None, _k=api_key, _m=model, _b=base_url: _call_openai_compatible(sp, ut, _k, _m, _b),
        })
...
def call_llm(system_prompt: str, user_text: str, response_schema: dict | None = None) -> str:
    ...
    for provider in providers:
        ...
        try:
            result = provider["call"](system_prompt, user_text, response_schema)
    ...
    if os.getenv("LLM_ALLOW_MOCK", "false").lower() != "true":
        raise RuntimeError("All configured real LLM providers failed; mock fallback is disabled") from last_error
