...
class TeacherResponse(BaseModel):
    """Respuesta pedagógica dirigida al profesor para un fragmento de clase.

    El texto se valida antes de publicarse; así el frontend nunca presenta
    metadatos técnicos del pipeline como si fueran una respuesta humana.
    """
    message: str = Field(min_length=1, max_length=1200)
    destination: Literal["chat", "board"] = "chat"
    component: Optional[UIComponentSchema] = None
    topic: str
    session_id: str
    ...
    def board_response_needs_component(self):
        if self.destination == "board" and self.component is None:
            raise ValueError("una respuesta para pizarra requiere componente")
        return self
